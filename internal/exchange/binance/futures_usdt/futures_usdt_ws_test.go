...
func TestBinanceFuturesUSDTWS_Kline(t *testing.T) {
	// 初始化日志系统并设置为DEBUG级别
	logger.Init()
	logger.SetLogLevel(logger.INFO)

	log.Printf("=== Binance USDT合约 WS Kline 测试 ===")
	m := manager.NewManager()
	ex := NewFuturesUSDTExchange(m.Cache())
	m.AddExchange(ex, 1)
	...
	// 订阅K线数据
	if err := m.SubscribeKline(ctx, schema.BINANCE, schema.FUTURESUSDT, []string{"BTCUSDT"}, schema.Interval1m); err != nil {
	...
	go func() {
		ticker := time.NewTicker(250 * time.Millisecond) // 每5秒检查一次
		defer ticker.Stop()
		...
		for {
			...
			case <-ticker.C:
				// 从缓存中读取kline数据
				if klines, exists := m.Cache().GetKline(schema.BINANCE, schema.FUTURESUSDT, "BTCUSDT", schema.Interval1m); exists && len(klines) > 0 {
...
func TestBinanceFuturesUSDTWS_Depth(t *testing.T) {
	// 初始化日志系统并设置为INFO级别
	logger.Init()
	logger.SetLogLevel(logger.WARN)

	log.Printf("=== Binance USDT合约 WS Depth 测试 ===")
	m := manager.NewManager()
	ex := NewFuturesUSDTExchange(m.Cache())
	m.AddExchange(ex, 1)
	ctx, cancel := context.WithCancel(context.Background())
	...
	// 订阅深度数据
	if err := m.SubscribeDepth(ctx, schema.BINANCE, schema.FUTURESUSDT, []string{"BTCUSDT"}); err != nil {
...
func printDepthData(cache *cache.MemoryCache, symbol string) {
	depth, ok := cache.GetDepth(schema.BINANCE, schema.FUTURESUSDT, symbol)
	if !ok {
	...
	// 打印买盘和卖盘各有多少档
	logger.Info("[%s] 买盘: %d档, 卖盘: %d档", symbol, len(depth.Bids), len(depth.Asks))

	//// 打印第5档买单和卖单
	//if len(depth.Bids) >= 5 {
	//	log.Printf("[%s] 第5档买单: %s @%s", symbol, depth.Bids[4].Price.String(), depth.Bids[4].Quantity.String())
	//} else {
	//	log.Printf("[%s] 买单数据不足5档", symbol)
	//}
	//
	//if len(depth.Asks) >= 5 {
	//	log.Printf("[%s] 第5档卖单: %s @%s", symbol, depth.Asks[4].Price.String(), depth.Asks[4].Quantity.String())
	//} else {
