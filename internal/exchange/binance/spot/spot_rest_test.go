...
// pass
func TestBinanceSpotREST_Depth(t *testing.T) {
	r := NewSpotREST()
...
// ExchangeInfo test
func TestBinanceSpotREST_ExchangeInfo(t *testing.T) {
	logger.Init()
	logger.SetLogLevel(logger.DEBUG)
	log.Printf("=== Binance现货 REST ExchangeInfo 测试 (DEBUG模式) ===")
	...
	if exchangeInfo.Market != schema.SPOT {
		t.Errorf("市场类型错误，期望: %s, 实际: %s", schema.SPOT, exchangeInfo.Market)
	}
	...
	for i := 0; i < limit; i++ {
		...
		log.Printf("%d. %s (%s/%s) - 数量精度:%d, 价格精度:%d",
			i+1, symbol.Symbol, symbol.Base, symbol.Quote,
